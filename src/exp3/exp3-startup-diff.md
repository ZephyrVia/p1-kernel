...
- [`generic_timer_init()`](src/kernel.c#L15)
...
实现位于 [`src/timer.c:generic_timer_init`](src/timer.c#L25)：

- [读取 `cntfrq_el0`](src/timer.c#L18)
- [调用 `gen_timer_init()`](src/timer.c#L31)
- [调用 `gen_timer_reset(interval)`](src/timer.c#L32)
...
exp2 没有 generic timer 初始化，因此不会周期性产生 timer IRQ。
...
- [调用 `handle_generic_timer_irq()`](src/irq.c#L52)

也就是说，`el1_irq` 是统一入口，`handle_irq()` 负责分发具体设备中断。

exp2 没有这个分发函数。

## 13. `handle_generic_timer_irq()`：处理 timer，并设置下一次中断

timer IRQ 最后进入 [`src/timer.c:handle_generic_timer_irq`](src/timer.c#L35)：

- [打印 timer interrupt 信息](src/timer.c#L37)
- [重新写 `CNTP_TVAL_EL0`](src/timer.c#L38)

重新写 `CNTP_TVAL_EL0` 是通过 [`gen_timer_reset(interval)`](src/timer.S#L23) 完成的。
...
| generic timer | 无 | 新增 [`generic_timer_init()`](src/timer.c#L25) |
...
| timer IRQ handler | 无 | 新增 [`handle_generic_timer_irq()`](src/timer.c#L35) |
